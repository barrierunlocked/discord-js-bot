...
const RateLimit = require('@root/src/database/schemas/RateLimitSettings');
const fs = require('fs');
...
module.exports = {
  ...
  slashCommand: {
    ...
    options: [
      ...
      {
        name: "rolestoadd",
        description: "List of roles to add, comma separated",
        type: ApplicationCommandOptionType.String,
        required: false
      },
      {
  ...
  async messageRun(message, args) {
    ...
    const rolestoadd = args[3] ? args[3].split(',').map(role => role.trim()) : null;
    const timelimit = args[4] ? parseInt(args[4]) : 0;
    const type = args[5] ? args[5] : "";
    ...
    if (action == 'set') {
      ...
      if (!rateLimitEntry) {
        ...
        rateLimitEntry.rolestoadd = rolestoadd;
      }
  ...
  async interactionRun(interaction) {
    ...
    const rolestoadd = getRoleIdsFromMentions(interaction.options.getString("rolestoadd"));
    let type = "";
